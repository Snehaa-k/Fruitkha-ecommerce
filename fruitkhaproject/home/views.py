...
def singleproduct(request, id):
    ...
    return render(
        request, "single-product.html", {"product": prdts, "variant": variant}
    )
...
def searchh(request):
    if "email" in request.session:
        ...
        return render(
            request, "shop.html", {"products": prdts, "search_query": search_query}
        )
    else:
...
def cart(request):
    ...
    subtotal = CartItem.objects.filter(user_id=user.id).aggregate(sum=Sum("total"))[
        "sum"
    ]
    final = subtotal
...
def add_wishlist(request, id):
    if request.method == "POST":
        ...
        if Wishlist.objects.filter(
            user_id=obj, product_id=product, vaiant_id=variant
        ).exists():
...
def add_cart(request, id):
    ...
    if request.method == "POST":
        ...
        cartitem = CartItem.objects.filter(
            user_id=user_instance, product_id=Product, Variant_id=obj
        )

        if cartitem.exists():
            messages.error(
                request, "the product is already in the cart just update the quantity"
            )
        ...
        if int(quantity1) <= obj.v_quantity:
            if quantity1 < 6:
                ...
                if not created:
                    messages.error(
                        request, "The product is already in the cart just update it"
                    )
                    return redirect("cart")
...
def userprofile(request):
    ...
    return render(
        request,
        "userprofile.html",
        {"user": user, "addres": addres, "wallet_amount": wallet_amount},
    )
    ...
    if "email" in request.session:
        ...
        if cartitem.exists():
            ...
            if pro.is_coupenapplyed:
                ...
                subtotal = CartItem.objects.filter(user_id=user.id).aggregate(
                    sum=Sum("total")
                )["sum"]
                final = subtotal
            elif prooffer:
    ...
    if request.method == "POST":
        ...
        for cart_item in caritem:
            if cart_item.c_quantity > cart_item.Variant_id.v_quantity:
                ...
                messages.error(
                    request, f"Product {cart_item.product_id.pname} is out of stock."
                )
                return redirect("checkout")
        if checkout.total_amount > 1000:
            messages.error(
                request,
                "you can'nt place order ,your amount should be limited to 1000..!",
            )
            return redirect("checkout")
        ad1 = ad.id
        if addres != None:
...
def pay_razorpay1(request):
    if request.method == "POST":
        ...
        checkout.delete()
        return JsonResponse(
            {"success": True, "amount": amount, "redirect_url": reverse("orderplace")}
        )
    ...
    if request.method == "POST":
        ...
        try:
            if total <= wallet_amt:
                ...
                wallet.save()
                return JsonResponse(
                    {"success": True, "redirect_url": reverse("orderplace")}
                )
            else:
...
def orderdetails(request):
    if "email" in request.session:
        ...
        wallet_amount = wallet.amountt
    return render(
        request, "orderdetails.html", {"order": order1, "wallet_amount": wallet_amount}
    )
...
def cancelorder1(request, id):
    ...
    if "email" in request.session:
        ...
        orderi.save()
        if (
            orderi.order_id.paymt_method == "razor_pay"
            or orderi.order_id.paymt_method == "wallet"
        ):
            ...
            if orderi.order_id.coupen_apply == "True":
                ...
                my_dict = Orderditem.objects.filter(
                    order_id_id=orderi.order_id.id
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if coupen.is_listed:
                ...
                JsonResponse(
                    {"success": False, "message": "Coupon applied succesfully"}
                )
                return redirect("checkout")
            else:
                JsonResponse(
                    {"success": False, "message": "you can't apply this coupon"}
                )
...
def invoice(request, id):
    ...
    return render(
        request, "invoice.html", {"order": order, "today": today, "orders": orders}
    )
